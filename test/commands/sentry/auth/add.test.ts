...
describe('auth:add', () => {
  ...
  // tested in plugin-lib's own test suite. Here we only test the Sentry-specific integration.
  it('exports correct integration points', async () => {
    const {default: AuthAdd} = await import('../../../../src/commands/sentry/auth/add.js')
    const {clearClients, testConnection} = await import('../../../../src/sentry/sentry-client.js')
