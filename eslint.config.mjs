...
const config = [
  ...
  // (e.g. the stale-sandbox sweep) that run via ts-node and aren't part of the
  ...
  // booleans) and the fixture data used by the Sentry API tests.
  {
    files: ['test/**/*.ts'],
    rules: {
      ...
      camelcase: 'off',
      // The `v` regex flag the rule now asks for needs an es2024 target; the
      // project compiles tests with the es2022 target from tsconfig.json.
      'require-unicode-regexp': 'off',
